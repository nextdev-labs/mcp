...
## Notes & next steps
- The in-memory rate limiter is suitable for single-instance deployments. For multi-instance or production usage behind load-balancers, consider a Redis (or other central store) backed limiter.
- Optionally add metrics (Prometheus) and logging for limiter hits and payload rejects.
- Consider exposing rate and size limits via environment variables or configuration file.

If you want, I can:
- Commit these changes and open a PR; or
- Replace the in-memory limiter with a Redis-backed implementation; or
- Add unit tests and CI steps for these checks.
